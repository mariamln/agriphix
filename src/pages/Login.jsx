import React, { useEffect, useState } from 'react';
import { Link, useNavigate, useSearchParams } from 'react-router-dom';
import { loginWithGoogle, loginWithEmail, registerWithEmail } from '@/utils/auth';
import { getAuthErrorMessage } from '@/utils/authErrors';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { useTranslation } from '@/i18n/LanguageContext';
import {
  VALUE_CHAIN_GROUPS,
  ROLE_DESCRIPTIONS,
  ROLES,
} from '@/constants/profile';
import { setIntendedRole } from '@/utils/intendedRole';
import { createPageUrl } from '@/utils';
import { getSafeReturnTo, normalizeAppPath } from '@/utils/routes';
import {
  Sprout,
  MapPin,
  ShoppingBag,
  Package,
  Layers,
  Factory,
  Store,
  User,
  Wrench,
  ArrowRight,
  ArrowLeft,
  CheckCircle,
} from 'lucide-react';

const ROLE_ICONS = {
  farmer: Sprout,
  landowner: MapPin,
  buyer: ShoppingBag,
  supplier: Package,
  aggregator: Layers,
  processor: Factory,
  retailer: Store,
  consumer: User,
  service_provider: Wrench,
};

function getRedirectPath(role, searchParams) {
  const fromQuery = getSafeReturnTo(searchParams);
  if (fromQuery) return fromQuery;
  // Always land on Dashboard after sign-in; role-specific pages are in the sidebar.
  return createPageUrl('Dashboard');
}

export default function Login() {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const [step, setStep] = useState('role');
  const [selectedRole, setSelectedRole] = useState(null);
  const [authMode, setAuthMode] = useState('signin');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [confirmPassword, setConfirmPassword] = useState('');
  const [fullName, setFullName] = useState('');
  const [authLoading, setAuthLoading] = useState(false);
  const [authError, setAuthError] = useState('');

  useEffect(() => {
    const roleParam = searchParams.get('role');
    if (roleParam && ROLES.includes(roleParam)) {
      setSelectedRole(roleParam);
    }
    const authErrorParam = searchParams.get('authError');
    if (authErrorParam) {
      setAuthError(decodeURIComponent(authErrorParam));
      setStep('auth');
    }
  }, [searchParams]);

  const roleLabel = (role) => t(`login.role.${role}`);

  const goToAuthStep = (role = selectedRole) => {
    if (role) setIntendedRole(role);
    setStep('auth');
  };

  const redirectPath = () => normalizeAppPath(getRedirectPath(selectedRole, searchParams));

  const handleGoogleSignIn = async () => {
    setAuthError('');
    setAuthLoading(true);
    try {
      const path = await loginWithGoogle(redirectPath());
      // Popup flow returns a path; redirect flow navigates away (null)
      if (path) navigate(path, { replace: true });
    } catch (err) {
      setAuthError(getAuthErrorMessage(err));
    } finally {
      setAuthLoading(false);
    }
  };

  const handleEmailSubmit = async (e) => {
    e.preventDefault();
    setAuthError('');

    if (authMode === 'signup') {
      if (password.length < 6) {
        setAuthError(t('login.passwordTooShort'));
        return;
      }
      if (password !== confirmPassword) {
        setAuthError(t('login.passwordMismatch'));
        return;
      }
    }

    setAuthLoading(true);
    try {
      const redirect = redirectPath();
      const path =
        authMode === 'signup'
          ? await registerWithEmail(email, password, fullName, redirect)
          : await loginWithEmail(email, password, redirect);
      navigate(path, { replace: true });
    } catch (err) {
      setAuthError(getAuthErrorMessage(err));
    } finally {
      setAuthLoading(false);
    }
  };

  return (
    <div className="min-h-full grid lg:grid-cols-[minmax(320px,42%)_1fr] bg-slate-50">
      <div className="hidden lg:flex flex-col justify-between bg-gradient-to-br from-emerald-800 via-emerald-900 to-teal-950 text-white p-10 xl:p-14">
        <div>
          <Link to="/" className="inline-flex items-center gap-3 mb-12">
            <div className="w-12 h-12 bg-emerald-500 rounded-xl flex items-center justify-center">
              <Sprout className="w-6 h-6" />
            </div>
            <div>
              <span className="text-2xl font-bold block">Agriphix</span>
              <span className="text-sm text-emerald-200">{t('landing.tagline')}</span>
            </div>
          </Link>
          <p className="text-amber-300 text-sm font-arabic tracking-widest mb-4">بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيمِ</p>
          <h1 className="text-3xl xl:text-4xl font-bold mb-4 leading-tight">{t('login.title')}</h1>
          <p className="text-emerald-100 text-lg leading-relaxed max-w-md">{t('login.subtitle')}</p>
          <ul className="mt-10 space-y-4 text-emerald-100/90">
            {[t('login.benefit1'), t('login.benefit2'), t('login.benefit3')].map((benefit) => (
              <li key={benefit} className="flex items-start gap-3">
                <CheckCircle className="w-5 h-5 text-emerald-300 shrink-0 mt-0.5" />
                <span>{benefit}</span>
              </li>
            ))}
          </ul>
        </div>
        <Link to="/" className="inline-flex items-center text-sm text-emerald-200 hover:text-white">
          <ArrowLeft className="w-4 h-4 mr-1" />
          {t('login.backHome')}
        </Link>
      </div>

      <div className="flex flex-col min-h-full bg-white">
        <main className="flex-1 px-4 sm:px-8 lg:px-10 xl:px-14 py-8 lg:py-12 overflow-y-auto">
          <div className="lg:hidden mb-8">
            <Link to="/" className="inline-flex items-center text-sm text-emerald-700 font-medium mb-4">
              <ArrowLeft className="w-4 h-4 mr-1" />
              {t('login.backHome')}
            </Link>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">{t('login.title')}</h1>
            <p className="text-gray-600 text-sm">{t('login.subtitle')}</p>
          </div>

          <div className="max-w-xl lg:max-w-2xl mx-auto lg:mx-0 lg:max-w-none w-full">
            {step === 'role' ? (
              <div className="space-y-8">
                {VALUE_CHAIN_GROUPS.map((group) => (
                  <section key={group.id}>
                    <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">
                      {t(group.labelKey)}
                    </h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {group.roles.map((role) => {
                        const Icon = ROLE_ICONS[role] || Sprout;
                        const selected = selectedRole === role;
                        return (
                          <button
                            key={role}
                            type="button"
                            onClick={() => setSelectedRole(role)}
                            className={`text-left p-4 rounded-xl border-2 transition-all ${
                              selected
                                ? 'border-emerald-500 bg-emerald-50 shadow-md ring-2 ring-emerald-200'
                                : 'border-gray-200 bg-white hover:border-emerald-300 hover:shadow-sm'
                            }`}
                          >
                            <div className="flex items-start gap-3">
                              <div
                                className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${
                                  selected ? 'bg-emerald-600 text-white' : 'bg-emerald-100 text-emerald-700'
                                }`}
                              >
                                <Icon className="w-5 h-5" />
                              </div>
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center justify-between gap-2 mb-0.5">
                                  <span className={`font-semibold capitalize ${selected ? 'text-emerald-800' : 'text-gray-900'}`}>
                                    {roleLabel(role)}
                                  </span>
                                  {selected && <CheckCircle className="w-4 h-4 text-emerald-600 shrink-0" />}
                                </div>
                                <p className="text-xs text-gray-500 leading-snug">{ROLE_DESCRIPTIONS[role]}</p>
                              </div>
                            </div>
                          </button>
                        );
                      })}
                    </div>
                  </section>
                ))}

                <div className="space-y-3 pt-2 border-t border-emerald-100">
                  <Button
                    onClick={() => goToAuthStep()}
                    disabled={!selectedRole}
                    size="lg"
                    className="w-full bg-emerald-600 hover:bg-emerald-700 py-6 text-base disabled:opacity-50"
                  >
                    {selectedRole
                      ? `${t('login.continueAs')} ${roleLabel(selectedRole)}`
                      : t('login.selectRole')}
                    {selectedRole && <ArrowRight className="w-5 h-5 ml-2" />}
                  </Button>
                  <p className="text-center text-sm text-gray-500">
                    {t('login.orGeneral')}{' '}
                    <button
                      type="button"
                      onClick={() => goToAuthStep(null)}
                      className="text-emerald-700 font-medium hover:underline"
                    >
                      {t('login.signInGeneral')}
                    </button>
                  </p>
                </div>
              </div>
            ) : (
              <Card className="border-emerald-100 shadow-lg">
                <CardContent className="p-8 space-y-6">
                  <button
                    type="button"
                    onClick={() => setStep('role')}
                    className="inline-flex items-center text-sm text-emerald-700 hover:text-emerald-800 font-medium"
                  >
                    <ArrowLeft className="w-4 h-4 mr-1" />
                    {t('login.changeRole')}
                  </button>

                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('login.authTitle')}</h2>
                    <p className="text-gray-600">{t('login.authSubtitle')}</p>
                  </div>

                  {selectedRole && (
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-50 border border-emerald-200">
                      {(() => {
                        const Icon = ROLE_ICONS[selectedRole] || Sprout;
                        return (
                          <div className="w-12 h-12 rounded-lg bg-emerald-600 text-white flex items-center justify-center">
                            <Icon className="w-6 h-6" />
                          </div>
                        );
                      })()}
                      <div>
                        <p className="text-xs text-emerald-700 font-medium uppercase tracking-wide">{t('login.yourRole')}</p>
                        <p className="text-lg font-semibold text-gray-900 capitalize">{roleLabel(selectedRole)}</p>
                      </div>
                    </div>
                  )}

                  <div className="flex rounded-lg border border-gray-200 p-1 bg-gray-50">
                    <button
                      type="button"
                      onClick={() => { setAuthMode('signin'); setAuthError(''); }}
                      className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${
                        authMode === 'signin'
                          ? 'bg-white text-emerald-700 shadow-sm'
                          : 'text-gray-600 hover:text-gray-900'
                      }`}
                    >
                      {t('login.tabSignIn')}
                    </button>
                    <button
                      type="button"
                      onClick={() => { setAuthMode('signup'); setAuthError(''); }}
                      className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${
                        authMode === 'signup'
                          ? 'bg-white text-emerald-700 shadow-sm'
                          : 'text-gray-600 hover:text-gray-900'
                      }`}
                    >
                      {t('login.tabSignUp')}
                    </button>
                  </div>

                  <form onSubmit={handleEmailSubmit} className="space-y-4">
                    {authMode === 'signup' && (
                      <div className="space-y-2">
                        <Label htmlFor="fullName">{t('login.fullName')}</Label>
                        <Input
                          id="fullName"
                          type="text"
                          autoComplete="name"
                          placeholder={t('login.fullNamePlaceholder')}
                          value={fullName}
                          onChange={(e) => setFullName(e.target.value)}
                        />
                      </div>
                    )}

                    <div className="space-y-2">
                      <Label htmlFor="email">{t('login.email')}</Label>
                      <Input
                        id="email"
                        type="email"
                        autoComplete="email"
                        required
                        placeholder={t('login.emailPlaceholder')}
                        value={email}
                        onChange={(e) => setEmail(e.target.value)}
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="password">{t('login.password')}</Label>
                      <Input
                        id="password"
                        type="password"
                        autoComplete={authMode === 'signup' ? 'new-password' : 'current-password'}
                        required
                        minLength={6}
                        placeholder={t('login.passwordPlaceholder')}
                        value={password}
                        onChange={(e) => setPassword(e.target.value)}
                      />
                    </div>

                    {authMode === 'signup' && (
                      <div className="space-y-2">
                        <Label htmlFor="confirmPassword">{t('login.confirmPassword')}</Label>
                        <Input
                          id="confirmPassword"
                          type="password"
                          autoComplete="new-password"
                          required
                          minLength={6}
                          value={confirmPassword}
                          onChange={(e) => setConfirmPassword(e.target.value)}
                        />
                      </div>
                    )}

                    {authError && (
                      <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2" role="alert">
                        {authError}
                      </p>
                    )}

                    <Button
                      type="submit"
                      size="lg"
                      disabled={authLoading}
                      className="w-full bg-emerald-600 hover:bg-emerald-700 py-6 text-base"
                    >
                      {authLoading
                        ? t('common.loading')
                        : authMode === 'signup'
                          ? t('login.signUpButton')
                          : t('login.signInButton')}
                    </Button>
                  </form>

                  <div className="relative">
                    <div className="absolute inset-0 flex items-center">
                      <span className="w-full border-t border-gray-200" />
                    </div>
                    <div className="relative flex justify-center text-xs uppercase">
                      <span className="bg-white px-2 text-gray-500">{t('login.orDivider')}</span>
                    </div>
                  </div>

                  <Button
                    type="button"
                    onClick={handleGoogleSignIn}
                    disabled={authLoading}
                    size="lg"
                    variant="outline"
                    className="w-full py-6 text-base border-gray-300 hover:bg-gray-50"
                  >
                    <svg className="w-5 h-5 mr-3" viewBox="0 0 24 24" aria-hidden="true">
                      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                    </svg>
                    {t('login.signInWithGoogle')}
                  </Button>
                </CardContent>
              </Card>
            )}
          </div>
        </main>
      </div>
    </div>
  );
}
